---
name: author-pr
description: Create ready-for-review pull requests on GitHub. Use when the branch is ready to open a PR; when the user asks to create, push, or open a PR; after local changes are committed; or when documenting user-facing features, bug fixes, performance optimizations, validation, changelog impact, or operational impact in a PR that follows repository conventions without AI attribution.
---
...
```text
feat(api): add token trend filters
fix(storage): preserve cursor session ids
docs: document agent contributor workflows
chore: tighten cleanup handling
```
...
**What.** Adds a repo-local contributor workflow plugin with agent skills.
...
- Vendors `$cross-harness-skills:start-work`, `$cross-harness-skills:crystallize`, `$cross-harness-skills:code-review`, and `$cross-harness-skills:author-pr` skills.
...
For user-facing features, demonstrate the feature in the PR description. Use the most natural form for the change: CLI commands, config snippets, API calls, screenshots, curl examples, or UI steps.
...
Then enable `cross-harness-skills` and invoke:

```text
Use $cross-harness-skills:author-pr to create this PR on GitHub.
```
````
...
With this setting, search only returns records from the current project.
````
...
- Adds developer-only PR authoring guidance for contributor agents.
...
Validated plugin metadata with `python3 -m json.tool .agents/plugins/marketplace.json` and `quick_validate.py plugins/cross-harness-skills/skills/author-pr`. Ran `make docs-build` successfully. Unit tests were not run; change is docs and plugin metadata only.
```

For stack-facing changes, include the repository's normal integration or smoke validation details, or state why that validation was not run.
...
Before this PR, records with missing workspace paths produced empty project
filters:
...
The fix normalizes the workspace path before writing the row. The new fixture
covers the missing-path case, and the focused storage test now passes.
````
...
Workload: 100k normalized events, local database, warm cache.
