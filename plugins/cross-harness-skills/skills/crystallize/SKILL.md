...
- `CodebaseResearch`: identify affected packages, modules, APIs, tests, docs, and local patterns. Return concrete file paths and constraints.
- `HistoryResearch`: use available session-history or agent-history tools when prior or active agent context may matter. Search past sessions by keywords, inspect relevant sessions, and return only actionable history.
...
```text
Research this rough implementation idea for the current repository.
Focus only on <role>.
...
```text
Attack this implementation plan for the current repository from the <role> perspective.
...
Include exact file paths where possible. Include commands with expected outcomes. For stack-facing changes, include the repository's normal integration or smoke validation. For public prior art that materially influenced the plan, include source links in `Context`.
